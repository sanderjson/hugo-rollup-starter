(function (factory) {
  ...
  */
  ...
  distribute = function distribute(v) {
    ...
    if (config.register) {
      config.register(gsap$1, Plugin, PropTween);
    }
  },
  ...
  */
  ...
  Tween.version = Timeline.version = gsap$1.version = "3.2.6";
  _coreReady = 1;
  ...
  */
  ...
  var pixiApp = function () {
    ...
    app.loader
      ...
      .load(function (loader, resources) {
        var bg = PIXI.Sprite.from(resources.bg.texture);
        bg.anchor.set(0);
        ...
        var pixiCircle = /*@__PURE__*/(function (superclass) {
          function pixiCircle(radius) {
          ...
          if ( superclass ) pixiCircle.__proto__ = superclass;
          pixiCircle.prototype = Object.create( superclass && superclass.prototype );
          pixiCircle.prototype.constructor = pixiCircle;

          pixiCircle.prototype.clear = function clear () {
            app.stage.removeChild(this);
          };

          pixiCircle.prototype.draw = function draw () {
            this.x = innerWidth - 4 * this.radius * Math.random();
            this.y = innerHeight - 4 * this.radius * Math.random();
          };

          pixiCircle.prototype.move = function move () {
            this.isMoving = true;
          ...
          pixiCircle.prototype.stop = function stop () {
            this.isMoving = false;

            this.tl.pause();
          };

          pixiCircle.prototype.toggle = function toggle () {
            // this.isMoving = false
            // // let time = this.radius - 2.6 * Math.random()
            // let time = 0.5
        ...
        var init = function () {
          ...
          for (var i$1 = 0, list = container.children; i$1 < list.length; i$1 += 1) {
            var child = list[i$1];
          ...
          document.addEventListener('togglePause', function (e) {
            paused();
          });
    ...
    var init = app.loader.onComplete.add(function () {}); // called once when the queued resources all load.
  };
  ...
  // button handlers
  //
  var buttonHomePause = document.getElementById('home-button-pause');
  var buttonHomeReset = document.getElementById('home-button-reset');

  var eventPause = new Event('togglePause', {});
  var eventReset = new Event('reset', {});

  buttonHomePause.addEventListener('click', function () {
    document.dispatchEvent(eventPause);
  });

  buttonHomeReset.addEventListener('click', function () {
    ...
    transitions: [
      {
        ...
        leave: function leave(ref) {
          ...
          console.log(header);
          gsapWithCSS.to(current.container, {
            duration: 1,
            opacity: 0,
            onStart: function () {
              // removePostList()
  ...
  // posts class add
  //
  var addCSSTransitionClass = function (event) {
    event.target.classList.add('on');
  };

  var removeCSSTransitionClass = function (event) {
    event.target.classList.remove('on');
  };

  var observePostList = function () {
    var posts = document.getElementsByClassName('io-post--link');
    for (var i = 0; i < posts.length; i++) {
      posts[i].setAttribute('style', 'opacity: 0;');
      posts[i].addEventListener('mouseenter', function (ev) { return addCSSTransitionClass(ev); });
      posts[i].addEventListener('mouseleave', function (ev) { return removeCSSTransitionClass(ev); }
      );
      io.observe(posts[i]);
    }
  ...
  var observerElements = document.getElementsByClassName('wrapper');
  ...
  var loop = function () {
    ...
    var box = el.querySelector('.child');
    el.tl = gsapWithCSS.timeline({paused: true});

    el.tl.to(box, 1, {y: -180, ease: Linear.easeNone});

    el.observer = new IntersectionObserver(function (entry) {
    ...
    el.progressTween = function () {
      //       parallax calculations
      var scrollPosition = window.scrollY + window.innerHeight;
      var elPosition = scrollPosition - el.offsetTop;
      var durationDistance = window.innerHeight + el.offsetHeight;
  ...
  for (var i$1 = 0, list = observerElements; i$1 < list.length; i$1 += 1) loop();
